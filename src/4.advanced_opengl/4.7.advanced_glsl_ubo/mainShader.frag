...
void main() {
    ...
    gl_FragColor += vec4(calcSpotLight(spotLight, normal, FragPos, viewDir), 1.0f);
    // gl_FragColor += vec4(calcPointLight(pointLight, normal, FragPos, viewDir), 1.0f);
    gl_FragColor += vec4(calcDirLight(dirLight, normal, viewDir), 1.0f);
}